...
- 完整格式：`proto:// host@ ip #port ?count=N ?life=N ?fallback`
...
- `?life=N`：可省略，默认为 10，表示单个会话最多存活多少秒，见 [#189](https://github.com/zfl9/chinadns-ng/issues/189)。
  - 0 表示不限制，只要上游不主动断开连接，对应 TCP/TLS 会话就一直存在。
- `?fallback`：可省略，无值的标志参数，表示该上游为 **备用（fallback）上游**，详见 [备用上游与健康检查](#备用上游与健康检查)。

### 备用上游与健康检查

- 默认情况下，组内的多个上游是 **并发查询、采纳最快结果**，没有主备之分。
- 给某个上游加上 `?fallback`，可将其标记为 **备用上游**：
  - 正常情况下，只有 **主上游**（未标记 `?fallback` 的上游）参与查询，行为与之前完全一致。
  - 当一个组内的 **所有主上游都无响应** 时，该组的查询会 **同时也发往备用上游** 兜底；主上游恢复后自动停止使用备用上游。
- 主上游是否健康，**完全由真实查询的结果被动判定**，不发送任何探测包，因此不增加额外查询、也不影响正常查询路径：
  - **判活**：只要收到来自某个主上游的可用响应（rcode 为 NOERROR/NXDOMAIN 且未截断），即认为该组主上游健康。
  - **判死**：当主上游持续 `timeout-sec` 秒（即 `--timeout-sec`，默认 5 秒）都收不到任何可用响应时，判定该组主上游不健康，开始 **主备齐发**。
  - 处于「主备齐发」状态时，主上游仍会收到真实查询，所以它一旦恢复，下一条查询的响应就会立刻让该组切回「仅主上游」——恢复检测同样是秒级、且无需探测。
- 只要组内出现 `?fallback` 上游就会自动启用该机制，无需额外开关；该组必须同时存在至少一个主上游，否则启动报错。
- 说明：
  - 启动时各组默认按「健康」对待，先只发主上游；若主上游一开始就不可用，约 `timeout-sec` 秒后才会判死并开始主备齐发，这期间发往主上游的查询会失败。
  - 「不健康」状态是 **非破坏性** 的（主上游照发、仅额外多发备用上游），但故障期间备用上游会承受与主上游同等的查询量。
  - 故障与恢复的检测窗口都由 `timeout-sec` 决定，调大查询超时会同步放宽该窗口。

例如：平时走 DoT 到 Google，当其整组无响应时，自动同时改用本地 `5300` 端口的上游兜底：

```shell
trust-dns tls://dns.google@8.8.8.8,udp://127.0.0.1#5300?fallback
```

`?fallback` 可用于 `china-dns`、`trust-dns` 以及 `group-upstream` 的任意上游。
...
- `cache-db` 启用缓存持久化，参数是 db 文件路径（可以不预先创建）。
  ...
  - 恢复缓存时仅校验记录的结构边界，不校验完整内容。请勿修改 db 文件，也不要跨平台共享 db 文件。
...
域名列表是一个纯文本文件，严格按照每行一个 **域名后缀** 解析。空白行会被忽略；`#` 表示注释开始，其后内容会被忽略，因此支持整行注释和行尾注释。每个非空、非注释行只能包含一个域名后缀，不能用空白字符分隔多个域名。域名后缀如 `baidu.com`、`www.google.com`，不能以 `.` 开头或结尾。出于性能考虑，最多允许 8 级域名（2025.03.27 版本之前是 4 级），超出部分将截断。
